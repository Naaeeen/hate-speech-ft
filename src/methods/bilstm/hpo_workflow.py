...
def extract_eval_metrics(result: dict[str, Any]) -> dict[str, Any]:
    ...
    raise KeyError("Could not find eval metrics in train_bilstm() result.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the full BiLSTM HPO workflow: tune, confirm, and final seed runs."
    )

    # Workflow arguments.
    parser.add_argument("--hpo_config", type=str, default=str(DEFAULT_SEARCH_SPACE_PATH))
    parser.add_argument("--search_space_name", type=str, default="bilstm")
    parser.add_argument("--n_trials", type=int, default=12)
    parser.add_argument("--hpo_seed", type=int, default=2026)
    parser.add_argument("--trial_output_root", type=str, default="outputs/bilstm_hpo")
...
def main() -> int:
    ...
    base_config["weight_decay"] = float(base_config.get("weight_decay", 0.0))
    ...
    trial_overrides = build_trial_overrides(
        base_experiment_id=args.trial_id or "bilstm_hpo",
        method=args.method or "bilstm",
    ...
    planned_tuning_configs = [{**base_config, **overrides} for overrides in trial_overrides]

    state: dict[str, Any] = {
        "workflow": "bilstm_hpo_tune_confirm_final",
        ...
        "weight_decay_policy": "fixed_0.01",
    ...
    print(f"Starting BiLSTM HPO workflow. Summary: {summary_path}", flush=True)
    ...
    # Stage 2: Confirm top configs by averaging 3 seeds.
    ...
    print("\nFinished BiLSTM HPO workflow.", flush=True)
