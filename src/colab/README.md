...
The notebook is just a run sheet. The actual settings live in each method's
`manual_config.py`, so we do not pass hyperparameters through command flags.
...
7. Run the config-preview cell. This prints the exact config Python will use.
8. Run the training cell once.
9. Open the run folder and copy the metrics you need.
...
If the values came from a saved CSV or old notes, check the field names before
copying. Transformer configs use `per_device_train_batch_size` and
...
Each printed trial has `manual_config_updates`. Copy one trial into the
method's `manual_config.py`, preview the config in the notebook, and run that
single setup.

## Where Results Are

The important files are in `CONFIG["output_dir"]`:
...
Use `result_summary.json` for manual CSV rows because it includes config,
metrics, runtime, model-selection info, and artifact paths in one place.
