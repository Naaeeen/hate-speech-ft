...
Each experiment run has one local output directory. That folder is the source
...
The following metrics are computed directly by the training code:
...
| parameter counts | `result_summary.json -> config -> trainable_params`, `total_params` | Also useful in aggregate tables. |
...
Local JSON stays focused on final eval/test/runtime/model-selection values. It
does not write full train/loss curve history for manual aggregation.

W&B records useful training curves when the method has an iterative training
loop. Single-stage Hugging Face Transformer methods emit
Trainer-native W&B history. Two-stage methods keep one parent W&B run and relay
stage-local histories as `stage1/*` and `stage2/*` keys. BiLSTM logs epoch-level
`train/loss` and `eval/*` curves from its custom PyTorch loop. TF-IDF has no
train/loss curve because it is a classical sklearn baseline, not an epoch-based
...
reported final tables numerically, use the same selected hyperparameters, same
seeds, same dataset/version environment, and `run_test = True` for final runs.

## Mapping To Reference Tables

The aggregate reference CSVs are named
`results/all/final_runs (1).csv`, `results/all/hpo_runs.csv`, and
`results/all/method_summary (1).csv`. This branch does not regenerate those
files. If you need comparable rows, copy fields from one run's
`result_summary.json`. Use the checked-in `results/` folder and shared Drive
folder to verify column names and recorded values, then rebuild only the rows
you need from new per-run outputs.

Manual runs keep the per-run fields needed for spreadsheet rows.

| Aggregate column | Manual-run source |
| --- | --- |
...
| `vocab_size` | `config -> vocab_size`, especially for BiLSTM train-split word vocab runs |
...
Do not add these bookkeeping columns to manual-run output files:
...
`manual_config_updates` in a deterministic seeded order. It does not launch
training, write `hpo_runs.csv`, create hash-suffixed trial IDs, record time
caps, or track failures. Copy the selected update values into the relevant
method's existing `manual_config.py`.
Set `run_test` manually only when that HPO run also needs prediction files for
AUROC, confusion matrices, or error examples.

Manual aggregate rebuild status:

| Artifact | Exact from per-run JSON | Derivable manually | Skipped in manual workflow |
| --- | --- | --- | --- |
| `final_runs.csv` metrics | method, seed, selected hyperparameters, eval/test metrics, runtime, GPU/memory, parameter counts, prediction/model paths | flattening nested `result_summary.json` into aggregate columns | `final_config_id`, `missing_config_hash`, status/failure/error fields, auto-created path strings |
| `hpo_runs.csv` metrics | validation metrics, sampled hyperparameters, runtime, GPU/memory, parameter counts, summary path for each run you actually execute | `search_method=random_search` if you want to label rows copied from HPO suggestions | auto-created trial IDs, time caps, eval-time fields, notes, status/failure/error fields |
| `method_summary.csv` metrics | mean/std and totals once all underlying run rows exist | best-HPO selection by validation metric, final seed counts, selected hyperparameters | extra bookkeeping fields |
| prediction ZIP contents | per-example predictions when `run_test = True` | confusion matrices, AUROC, error examples, and analysis JSON from prediction files | the ZIP file itself and HPO metadata unless you assemble it manually |
...
When reusing saved `selected_hyperparams_json` values from the result CSVs, most
keys can be copied directly to `manual_config.py`. These fields need manual
translation or can be treated as fixed derived values:

| Saved hyperparameter field | Manual handling |
| --- | --- |
| `save_final_model` | Do not add this saved CSV key to `manual_config.py`. Configs use the inverse key `no_save_final_model`; keep `no_save_final_model = False` to save the final model. |
| `bf16` / `fp16` | Prefer `mixed_precision = "bf16"`, `"fp16"`, or `"none"`. |
| `greater_is_better` | Do not add this saved CSV key to `manual_config.py`. Configs use the inverse key `lower_is_better`; keep `lower_is_better = False` for macro-F1 selection. |
| Transformer `batch_size` / `eval_batch_size` | Use `per_device_train_batch_size` and `per_device_eval_batch_size` in `manual_config.py`; result JSON records `batch_size` names inside `hyperparameters` for table compatibility. |
| Full FT / LoRA `epochs` | Use `num_train_epochs` in `manual_config.py`; result JSON records `epochs` for table compatibility. |
...
| BiLSTM `optim` | Fixed to AdamW/`adamw_torch` in the BiLSTM runner. |
| BiLSTM `lr_scheduler_type` | Fixed to the same linear scheduler behavior in the BiLSTM runner. |
| BiLSTM `mixed_precision` | Fixed to `none`; BiLSTM does not use mixed precision. |
| BiLSTM `gradient_checkpointing` | Fixed unsupported/false; it does not affect the BiLSTM run. |

## W&B

W&B is a tracking view, not the source of truth. Runs log final
validation/test metrics, runtime fields, model-selection metadata, and normal
training curves when a method has an iterative training loop. Local JSON files
should be kept with the run.

Important W&B key patterns:

```text
train/loss                 # BiLSTM and Trainer-backed methods
train/global_step          # BiLSTM
eval/f1_macro
eval/accuracy
test/f1_macro
test/accuracy
...
Two-stage methods relay stage-local Trainer histories into one parent W&B run:
...
W&B groups/tags/model-log settings and aggregate CSV writers are intentionally
outside this manual workflow.
