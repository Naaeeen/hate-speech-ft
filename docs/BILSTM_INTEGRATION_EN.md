...
- The model still uses randomly initialized embeddings, not a pretrained
  DistilBERT encoder.
- The tokenizer is a Bi-LSTM-owned word vocabulary fitted on the preprocessed
  train split only. Validation and test text are encoded with `<unk>` for
  out-of-vocabulary words, so the tokenizer does not leak eval/test tokens.
...
The tokenizer builds a deterministic word-level vocabulary from
`train_data.records` after shared HateXplain preprocessing. It reserves:
...
Then it keeps train-split tokens with frequency at least
`--tokenizer_min_freq`, sorted by descending frequency and token text, capped by
`--max_vocab_size`. It converts text to:
...
The Bi-LSTM then trains a random embedding table over those method-local token
ids. Saved tokenizer artifacts include `vocab.json` and
`tokenizer_config.json`; the config records the vocab checksum and tokenizer
...
5. `tokenizer.py` fits a Bi-LSTM word vocabulary from the train split only.
...
trial cap: configs/search_spaces.json -> trial_caps.bilstm = 20
...
For the current Bi-LSTM tokenizer, `tokenizer_min_freq=2` and
`max_vocab_size=30000` are fixed catalog fields, not searched HPO dimensions.
...
<output_dir>/model.pt
<output_dir>/tokenizer/vocab.json
...
- Tokenizer policy fields are recorded in `resolved_config.json`; tokenizer
  hyperparameters are part of the Bi-LSTM config hash identity.
...
- Do not fit the Bi-LSTM tokenizer on validation or test records.
...
- Do not change tokenizer policy without updating `config_hash_keys.bilstm`,
  tests, and the integration notes; tokenizer changes alter the Bi-LSTM
